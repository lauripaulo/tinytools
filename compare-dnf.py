...
def compare_package_lists(original, new):
    ...
    """
    original_set = set(pkg["name"] for pkg in original)
    new_set = set(pkg["name"] for pkg in new)
...
def main():
    ...
    try:
        ...
        added_packages, removed_packages = compare_package_lists(
            original_packages, new_packages
        )
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='_added_packages.txt', delete=False) as added_file:
            ...
            command = f"sudo dnf --skip-unavailable install "
